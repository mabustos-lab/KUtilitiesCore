...
namespace KUtilitiesCore.MVVM.Messaging
{
    public class Messenger
    {
        ...
        /// <summary>
        /// Obtiene la instancia predeterminada de Messenger, que permite registrar y enviar
        /// mensajes de forma estática.
        /// </summary>
        public static Messenger Default
        {
            get
            {
                if (_defaultInstance == null) _defaultInstance = new Messenger();
                return _defaultInstance;
            }
        }

        /// <summary>
        /// Proporciona una forma de invalidar la instancia de Messenger.Default con una instancia
        /// personalizada, por ejemplo, con fines de pruebas unitarias.
        /// </summary>
        ...
        /// <summary>
        /// Registra un destinatario para un tipo de mensaje <typeparamref name="TTypeMessage"/>. El
        /// parámetro de acción se ejecutará cuando se envíe un mensaje correspondiente. Consulte el
        /// parámetro receiveDerivedMessagesToo para obtener detalles sobre cómo se pueden recibir
        /// también los mensajes derivados de <typeparamref name="TTypeMessage"/> (o, si
        /// <typeparamref name="TTypeMessage"/> es una interfaz, mensajes que implementan
        /// <typeparamref name="TTypeMessage"/>).
        /// <para>
        /// El registro de un destinatario no crea una referencia dura a él, por lo que si se
        /// elimina este destinatario, no se produce ninguna pérdida de memoria.
        /// </para>
        /// </summary>
        ...
        /// <param name="token">
        /// Un token para un canal de mensajería. Si un destinatario se registra con un token y un
        /// remitente envía un mensaje con el mismo token, este mensaje se entregará al
        /// destinatario. Otros destinatarios que no usaron un token al registrarse (o que usaron un
        /// token diferente) no recibirán el mensaje. Del mismo modo, los mensajes enviados sin
        /// ningún token, o con un token diferente, no se entregarán a ese destinatario.
        /// </param>
        /// <param name="receiveDerivedMessagesToo">
        /// Si es true, los tipos de mensajes derivados de <typeparamref name="TTypeMessage"/>
        /// también se transmitirán al destinatario. Por ejemplo, si SendOrderMessage y
        /// ExecuteOrderMessage derivan de OrderMessage, registrarse en OrderMessage y establecer
        /// receiveDerivedMessagesToo en true enviará SendOrderMessage y ExecuteOrderMessage al
        /// destinatario que se registró.
        /// <para>
        /// Además, si <typeparamref name="TTypeMessage"/> es una interfaz, los tipos de mensajes
        /// que implementan <typeparamref name="TTypeMessage"/> también se transmitirán al
        /// destinatario. Por ejemplo, si SendOrderMessage y ExecuteOrderMessage implementan
        /// IOrderMessage, registrarse en IOrderMessage y establecer receiveDerivedMessagesToo en
        /// true enviará SendOrderMessage y ExecuteOrderMessage al destinatario que se registró.
        /// </para>
        /// </param>
        /// <param name="action">
        /// La acción que se ejecutará cuando se envíe un mensaje de tipo TMessage.
        /// </param>
        public virtual void Register<TTypeMessage>(object recipient, Action<TTypeMessage> action, bool receiveDerivedMessagesToo=false, object token=null)
        {
            Dictionary<Type, List<Messenger.WeakActionAndToken>> types;
            List<WeakActionAndToken> item;
            Type type = typeof(TTypeMessage);
            lock (_registerLock)
            {
                ...
                else
                {
                    ...
                    types = _recipientsOfSubclassesAction;
                }
                ...
                {
                    item = new List<WeakActionAndToken>();
                    types.Add(type, item);
                }
                WeakAction<TTypeMessage> weakAction = new WeakAction<TTypeMessage>(recipient, action);
        ...
        /// destinatarios que se registraron para este tipo de mensaje mediante uno de los métodos Register.
        /// </summary>
        ...
        {
            this.SendToTargetOrType<TTypeMessage>(message, null, null);
        }

        /// <summary>
        /// Envía un mensaje a los destinatarios registrados. El mensaje solo llegará a los
        /// destinatarios que se registraron para este tipo de mensaje mediante uno de los métodos
        /// Register y que son de targetType.
        /// </summary>
        /// <typeparam name="TTypeMessage">The type of message that will be sent.</typeparam>
        /// <typeparam name="TTarget">
        /// El tipo de destinatarios que recibirán el mensaje. El mensaje no se enviará a
        /// destinatarios de otro tipo.
        /// </typeparam>
        /// <param name="message">El mensaje que se enviará a los destinatarios registrados.</param>
        public virtual void Send<TTypeMessage, TTarget>(TTypeMessage message)
        {
            this.SendToTargetOrType<TTypeMessage>(message, typeof(TTarget), null);
        }
        ...
        {
            this.SendToTargetOrType<TTypeMessage>(message, null, token);
        }

        /// <summary>
        /// Cancela completamente el registro de un destinatario del mensajer. Después de ejecutar
        /// este método, el destinatario ya no recibirá ningún mensaje.
        /// </summary>
        ...
        /// tipo <typeparamref name="TTypeMessage"/>, pero seguirá recibiendo otros tipos de mensajes (si se registró para
        /// ellos anteriormente).
        /// </summary>
        /// <param name="recipient">El destinatario que debe estar dado de baja.</param>
        ...
        /// </typeparam>
        public virtual void Unregister<TTypeMessage>(object recipient)
        {
            Unregister<TTypeMessage>(recipient, null);
        }
        ...
        private static void CleanupList(IDictionary<Type, List<WeakActionAndToken>> items)
        {
            if (items != null)
            {
                lock (items)
                {
                    if (items != null && items.Count > 0)
                    {
                        ...
                        {
                            item.Value.RemoveAll(x => x.Action == null || !x.Action.IsAlive);
                            if (item.Value.Count == 0) typesToRemove.Add(item.Key);
                        }
                        foreach (Type typeKey in typesToRemove)
                        {
                            items.Remove(typeKey);
                        }
                    }
                }
            }
        ...
        private static void SendToList<TTypeMessage>(TTypeMessage message,
            ...
            object token)
        {
            ...
            if (list != null)
            {
                List<WeakActionAndToken> weakActionAndTokens = list.Take(list.Count()).ToList();
                foreach (WeakActionAndToken weakActionAndToken in weakActionAndTokens)
                {
                    IExecuteWithObject action = weakActionAndToken.Action as IExecuteWithObject;
                    flag = action != null
                        && weakActionAndToken.Action.IsAlive
                        && weakActionAndToken.Action.Target != null
                        && (messageTargetType == null || weakActionAndToken.Action.Target.GetType() == messageTargetType || Implements(weakActionAndToken.Action.Target.GetType(), messageTargetType))
                        && ((weakActionAndToken.Token == null && token == null) || (weakActionAndToken.Token != null && weakActionAndToken.Token.Equals(token)));
                    if (flag)
                    {
                        if (_ctx != null)
                        {
                            _ctx.Send(delegate { action.ExecuteWithObject(message); }, null);
                        }
                        else
                        {
                ...
                lock (lists)
                {
                    foreach (Messenger.WeakActionAndToken item in lists[type])
                    {
                        flag = item.Action is WeakAction<TMessage> weakAction
                            ...
                            && (action == null ? true : action == weakAction.Action);
                        if (flag)
                        {
        ...
        private void SendToTargetOrType<TTypeMessage>(TTypeMessage message,
            Type messageTargetType,
            object token)
        {
